...
// if more than "dynamic_threshold"% of its hits are dynamic the leg is considered to be dynamic
    ...
    for (int loop=0; loop<nb_cluster; loop++)//loop over all the clusters
        if (cluster_size[loop] > leg_size_min && cluster_size[loop] < leg_size_max)//the size of the current cluster is higher than "leg_size_min" and lower than "leg_size_max" then the current cluster is a leg
	{
		leg_detected[nb_leg_detected] = cluster_middle[loop];
		leg_cluster[nb_leg_detected] = loop;
		leg_dynamic[nb_leg_detected] = cluster_dynamic[loop];			
