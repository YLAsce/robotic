...
public:
    ...
    void perform_clustering()
    {
        ...
        //...
        cluster[nb_beams - 1] = nb_cluster; //@@IMPD

        if (dynamic[nb_beams - 1]) //@@IMPD
            nb_dynamic++;          //@@IMPD

        end = nb_beams - 1;                                      //@@IMPD                                                                                                                                   //@@IMPD
        float xcs = current_scan[start].x - current_scan[end].x; //@@IMPD                                                                                                //@@IMPD
        ...
        colors[nb_pts].r = 1;   //@@IMPD
        colors[nb_pts].g = 0;   //@@IMPD
        colors[nb_pts].b = 0;   //@@IMPD
        colors[nb_pts].a = 1.0; //@@IMPD
        nb_pts++;               //@@IMPD

        // textual display
        ROS_INFO("cluster[%i] = (%f, %f): hit[%i](%f, %f) -> hit[%i](%f, %f), size: %f, dynamic: %i, %i", nb_cluster, //@@IMPD
                 cluster_middle[nb_cluster].x,
                 cluster_middle[nb_cluster].y,
                 start,
                 current_scan[start].x,
        ...
        for (int loop_leg1 = 0; loop_leg1 < nb_legs_detected; loop_leg1++)                 // loop over all the legs
            ...
            {                                                                              // if the distance between two legs is lower than "legs_distance_max"
                ...
                if (xx * xx + yy * yy < legs_distance_max * legs_distance_max)    //@@IMPD
                {
                    ...
                    person_detected[nb_persons_detected].x = (leg_detected[loop_leg1].x + leg_detected[loop_leg2].x) / 2; //@@IMPD
                    person_detected[nb_persons_detected].y = (leg_detected[loop_leg1].y + leg_detected[loop_leg2].y) / 2; //@@IMPD
                                                                                                                          // we update the person_dynamic table to know if the person is moving or not
                    if (leg_dynamic[loop_leg1] && leg_dynamic[loop_leg2])                                                 //@@IMPD
                        person_dynamic[nb_persons_detected] = true;                                                       //@@IMPD
                    else                                                                                                  //@@IMPD
                        person_dynamic[nb_persons_detected] = false;                                                      //@@IMPD
    ...
    void track_a_moving_person()
    {
        ...
        {
            float xx = person_detected[loop_detection].x - moving_person_tracked.x;//@@IMPD
            float yy = person_detected[loop_detection].y - moving_person_tracked.y;//@@IMPD
            float current_dist = sqrt(xx*xx + yy*yy);//@@IMPD//= ...; EUCLIDIAN DISTANCE between the moving_person_tracked and the current_person_detected
            ROS_INFO("distance with [%i] = %f", loop_detection, current_dist);//@@IMPD
            if (current_dist < distance_min)//@@IMPD
            {
                // we store the info about the closest detection
                associated = true;//@@IMPD
                distance_min = current_dist;//@@IMPD
                index_min = loop_detection;//@@IMPD
